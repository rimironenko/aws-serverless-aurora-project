...
class PutAuroraItemFunctionTest {
    ...
    @Test
    public void shouldCreateAuroraItemIfRequestIsValid() {
        ...
        try (MockedStatic<DependencyFactory> dependencyFactoryMockedStatic = mockStatic(DependencyFactory.class)) {
            ...
            assertEquals(PutAuroraItemFunction.HTTP_STATUS_CODE_CREATED, (int) response.getStatusCode());

        }
    }

    @Test
    public void shouldNotCreateAuroraItemIfRequestIsNotValid() {
        when(client.executeStatement(any(ExecuteStatementRequest.class))).thenReturn(statementResponse);
        Book testBook = new Book(TEST_ID);
        testBook.setAuthor(TEST_AUTHOR);
        testBook.setName(TEST_NAME);
        ...
        try (MockedStatic<DependencyFactory> dependencyFactoryMockedStatic = mockStatic(DependencyFactory.class)) {
            ...
            PutAuroraItemFunction handler = new PutAuroraItemFunction();
            APIGatewayProxyResponseEvent response = handler.handleRequest(request, context);
            assertEquals(PutAuroraItemFunction.HTTP_STATUS_CODE_NO_CONTENT, (int) response.getStatusCode());
